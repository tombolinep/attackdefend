...
class GameOverView:
    ...
    def draw(self, screen, model):
        ...
        text_rect = text_surface.get_rect(center=(self.settings.STATS_WIDTH + self.settings.MAIN_GAME_WIDTH / 2, self.settings.SCREEN_HEIGHT / 2 - 100))
        text_shadow_rect = text_shadow_surface.get_rect(center=(text_rect.centerx + 3, text_rect.centery + 3))

        score_rect = score_surface.get_rect(center=(self.settings.STATS_WIDTH + self.settings.MAIN_GAME_WIDTH / 2, self.settings.SCREEN_HEIGHT / 2 - 20))
        score_shadow_rect = score_shadow_surface.get_rect(center=(score_rect.centerx + 2, score_rect.centery + 2))

        instruction_rect = instruction_surface.get_rect(
            center=(self.settings.STATS_WIDTH + self.settings.MAIN_GAME_WIDTH / 2, self.settings.SCREEN_HEIGHT / 2 + 50))
